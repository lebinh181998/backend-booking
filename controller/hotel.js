...
exports.getHotelsBySearchKeys = async (req, res, next) => {
  ...
  const roomCount = req.body.roomCount ? req.body.roomCount : 1;
  const peopleCount = req.body.peopleCount ? req.body.peopleCount : 1;
  const minPrice = req.body.minPrice;
  const maxPrice = req.body.maxPrice;
  const hotelID = req.body.hotelID;
  const peopleInARoom = Math.ceil(Number(peopleCount) / Number(roomCount));
  ...
  for (const item of hotelsFilter) {
    ...
    // //------------bản mới
    // //--(
    // //lấy các phòng có maxpeople đạt yêu cầu số người(lấy sô người chia số phòng để lấy số lượng người trong 1 phòng)
    // item.rooms = item.rooms.filter((item) => item.maxPeople >= peopleInARoom);
    // //lấy số phòng còn trống sau các yêu cầu lọc
    // let roomToBook = [];
    // item.rooms.map(
    //   (item) => (roomToBook = [...roomToBook, ...item.roomNumbers])
    // );

    // //tạo dữ liệu khách sạn mới thêm vào mảng hotelsRoom nếu đạt yêu cầu số phòng
    // if (roomToBook >= roomCount) {
    //   const hotel = await {
    //     ...(item._doc ? item._doc : item),
    //   };
    //   console.log(hotelsRoom);
